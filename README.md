...
| 🛡️ 安全 | 输入清洗（Prompt 注入 / SQL 注入）、PII 脱敏三模式、敏感操作二次确认、限流 |
...
| 依赖 | 版本/位置 |
|---|---|
| JDK | **Java 17**（⚠️ 系统默认 JDK 25 会破坏 Lombok，必须切 JDK 17） |
...
- 运行时回归：`node tools/<script>.js`（13 个脚本，覆盖接口契约、并发不超卖、安全护栏、RAG 降级、性能等）
